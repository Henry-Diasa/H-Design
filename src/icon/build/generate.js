...
fs.outputFile('../react-icon/context.js', contextJsx, (err) => {
  if (err) return;
  console.log('Generate context file success!');
});
...
fs.outputFile(
  ...
  (err) => {
    if (err) return;
    console.log('Generate commonjs context file success!');
  },
);
...
fs.outputFile('../react-icon/context.d.ts', dTs, (err) => {
  if (err) return;
  console.log('Generate context ts file success!');
});
